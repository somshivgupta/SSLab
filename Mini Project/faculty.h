...
bool offeringCourses(int sockfd,int facultyID)
{
    ...
    if (lockstatus == -1)
    {
        perror("Error while obtaining read lock (offering course) on the course file!");
        return false;
    }
    else
    	printf("\nRead lock for course\n");
